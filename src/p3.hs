...
emptyElem = Elem Nothing Nothing

data St = St {
  _pos      :: (Int, Int),
  _distance :: Int,
  _grid     :: M.Map (Int, Int) Elem
} deriving (Show)

makeLenses ''St


convert p = map f $ map T.unpack (T.splitOn (T.pack ",") p)
  where f x = (head x, read (tail x) :: Int)


move :: Path -> (Char, Int) -> State St ()
move ls (d, l) = do
  (x', y') <- use pos
  dist <- use distance
  case d of
    'R' -> do
      mapM_ (\(dd, x) -> grid . at x . non emptyElem %= f dd) (zip [dist..] [(x, y') | x <- [x'..(x'+l)]])
      pos . _1 %= (+l)
    'L' -> do
      mapM_ (\(dd, x) -> grid . at x . non emptyElem %= f dd) (zip [dist..] [(x, y') | x <- reverse [(x'-l)..x']])
      pos . _1 %= flip (-) l
    'U' -> do
      mapM_ (\(dd, x) -> grid . at x . non emptyElem %= f dd) (zip [dist..] [(x', y) | y <- [y'..(y'+l)]])
      pos . _2 %= (+l)
    'D' -> do
      mapM_ (\(dd, x) -> grid . at x . non emptyElem %= f dd) (zip [dist..] [(x', y) | y <- reverse [(y'-l)..y']])
      pos . _2 %= flip (-) l
  distance %= (+l)

  where f ddd e = case ls of
                    Path1 -> case e ^. p1 of
                               Nothing -> e & p1 .~ Just ddd
                               Just _ -> e
                    Path2 -> case e ^. p2 of
                               Nothing -> e & p2 .~ Just ddd
                               Just _ -> e

wholePath p = execState (traverse (move Path1) p) (St (1,1) 0 M.empty)

wholePath' p1 p2 =
  let first  = execState (traverse (move Path1) p1) (St (1,1) 0 M.empty)
      second = execState (traverse (move Path2) p2) (St (1,1) 0 (first ^. grid))
  in second

shortest x = sort $ map f $ M.elems $ M.filter (\x -> isJust (x ^. p1) && isJust (x ^. p2)) (x ^. grid)
  where f (Elem (Just x) (Just y)) = x + y

--testpath1 = convert "R8,U5,L5,D3"
--testpath2 = convert "U7,R6,D4,L4"
--
testpath1 = convert "R75,D30,R83,U83,L12,D49,R71,U7,L72"
testpath2 = convert "U62,R66,U55,R34,D71,R55,D58,R83"
